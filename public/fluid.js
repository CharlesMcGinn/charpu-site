...
(function(w) {
    ...
    */
    var canvas_width = window.innerWidth; // 500; //Needs to be a multiple of the resolution value below.
    var canvas_height = window.innerHeight; // 500; //This too.
    ...
    var num_cols = Math.ceil(canvas_width / resolution); //This value is the number of columns in the grid.
    var num_rows = Math.ceil(canvas_height / resolution); //This is number of rows.
    ...
    */
    function init() {
        ...
        if (!canvas) {
            console.warn("Fluid: #fluid-canvas not found. Skipping initialization.");
            return;
        }

        ctx = canvas.getContext("2d");
        if (!ctx) {
            console.warn("Fluid: unable to get 2D context. Skipping initialization.");
            return;
        }
    ...
    */
    function update_particle() {
        ...
        for (i = 0; i < particles.length; i++) {
            ...
            var dxm = p.x - mouse.x;
            var dym = p.y - mouse.y;
            var distToMouse = Math.sqrt(dxm * dxm + dym * dym);

            //Randomize color if the mouse is down and the particle is within the pen size radius of the mouse
            // if (mouse.down && distToMouse < pen_size) {
            //     p.targetColor = {
            //         r: 100 + Math.random() * 255,
            //         g: 100 + Math.random() * 255,
            //         b: 100 + Math.random() * 255
            //     };
            //     p.colorMix = 1;
            // }

            if (distToMouse < pen_size) {
                p.targetColor = {
                    r: 100 + Math.random() * 255,
                    g: 100 + Math.random() * 255,
                    b: 100 + Math.random() * 255
            ...
            p.colorMix *= 0.98; // fade speed

            //ctx.fillStyle = "#00FFFF";
            //For color changing, this sets the base color and then mixes it with the target color based on the colorMix value, which is reduced over time to create a fading effect back to the base color.
            var base = { r: 174, g: 0, b: 255 }; // cyan
            var t = p.colorMix;

            var r = base.r + (p.targetColor.r - base.r) * t;
            var g = base.g + (p.targetColor.g - base.g) * t;
            var b = base.b + (p.targetColor.b - base.b) * t;
            ...
            if (p.x >= 0 && p.x < canvas_width && p.y >= 0 && p.y < canvas_height) {
                ...
                */
                ...
                if (dist > limit) {
                    ...
                    aren't moving.
                    */
                    ctx.lineTo(p.x + limit * 4, p.y + limit * 4);
    ...
    var mouse_yv = mouse.y - mouse.py;
        ...
        for (i = 0; i < vec_cells.length; i++) {
            ...
            for (j = 0; j < cell_datas.length; j++) {
                ...
                // }
                var strength = mouse.down ? 1 : 0.3;
                change_cell_velocity(cell_data, mouse_xv * strength, mouse_yv * strength, pen_size);
                //change_cell_velocity(cell_data, mouse_xv, mouse_yv, pen_size);
    ...
    */
    ...
    function particle(x, y) {
        ...
        //For color changing
        this.color = { r: 174, g: 0, b: 255 }; // base cyan
        this.targetColor = { r: 174, g: 0, b: 255 };
        this.colorMix = 0; // 0 → base, 1 → target

        //Set up drift for motion
        this.driftX = (Math.random() - 0.5) * 0.2;
        this.driftY = -Math.random() * 0.3;
