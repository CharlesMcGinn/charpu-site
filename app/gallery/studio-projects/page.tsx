...
export default function FeaturedWorksPage() {
  ...
  useEffect(() => {
    const fetchFilms = async () => {
      setLoading(true)
      try {
        const response = await fetch('/api/sanity/content?type=studio-projects')
  ...
  return (
    <>
      ...
      <main className="bg-black">
        ...
        >
          ...
          <div className="relative z-10 h-full flex items-end p-12">
            <h1 className="text-4xl md:text-5xl font-bold text-white">Studio Projects</h1>
          </div>
        </div>
                        ...
                        <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-between p-6">
                          ...
                          <div className="flex-1 flex items-center justify-center">
                            <div className="flex gap-4">
                              <button onClick={() => setSelectedVideoFilm(film)} className="bg-purple-500 text-white py-2 px-4 rounded">Watch</button>
                              <button onClick={() => setSelectedFilmId(film._id)} className="border border-white/30 text-white py-2 px-4 rounded">Info</button>
                            </div>
                          </div>
      ...
      {/* Video Overlay for Watch button */}
      {selectedVideoFilm && (
